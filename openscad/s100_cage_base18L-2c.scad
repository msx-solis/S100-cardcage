...
difference(){
  ...
  union(){
    ...
    translate ([-19.05,0,0]) bosses(mid_holes, boss_dia, bp_hole_row3, 0, base_thick+boss_thick);          // Top boss between slots 4 & 5
    ...
    translate ([70,229,10]) rotate([90,0,0]) cylinder(h=10, d=3.1, center=false);
    translate ([98,229,10]) rotate([90,0,0]) cylinder(h=10, d=3.1, center=false);
  ...
  translate ([-19.05,0,0]) bosses(mid_holes, screw_threads, bp_hole_row3, -1, base_thick+boss_thick+2);      // Top hole between slots 4 & 5
  //agujeros altair izdo
  bosses(slot_ofs,  screw_threads, bp_hole_row3, -1, base_thick+boss_thick+2);      // Top card connector holes
  //agujeros imsai izdo
  translate ([19.05,0,0]) bosses(tm_holes,  screw_threads, bp_hole_row4, -1, base_thick+boss_thick+2);      // Top mounting holes (minus error)
